...
def seed_everything(seed=1454):
    ...
    # may be able to set these globally
...
def typical_top_k_filtering(logits, top_k=0, top_p=0.0, min_tokens_to_keep=1, filter_value=-1E12):
...
class CrossAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.ln_q = LayerNorm(input_q_dim,
                              device=device,
                              dtype=dtype)
        self.ln_k = LayerNorm(input_kv_dim // 2,
                              device=device,
                              dtype=dtype)
        # do it this way to handle odd sizes
        self.ln_v = LayerNorm(input_kv_dim - input_kv_dim // 2,
                              device=device,
                              dtype=dtype)
    ...
    def forward(self,
                ...
                q_positions,
                kv_positions,):
        s_ = inputs_kv.shape[-1] // 2
        attn, attn_state = self.attn(self.ln_q(inputs_q),
                                     torch.concat((self.ln_k(inputs_kv[..., :s_]),
                                                   self.ln_v(inputs_kv[..., s_:])), axis=-1),
                                     mask=mask,
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = Attention(input_dim,
                              ...
                              device=device,
                              dtype=dtype)
        self.ln_q = LayerNorm(input_dim // 3,
                              device=device,
                              dtype=dtype)
        self.ln_k = LayerNorm(input_dim // 3,
                              device=device,
                              dtype=dtype)
        # do it this way to handle any leftover dims
        self.ln_v = LayerNorm(input_dim - 2 * (input_dim // 3),
                              device=device,
                              dtype=dtype)
    ...
    def forward(self,
                ...
                q_positions,
                kv_positions,):
        s_ = inputs_qkv.shape[-1] // 3
        q = self.ln_q(inputs_qkv[..., :s_])
        k = self.ln_k(inputs_qkv[..., s_:2*s_])
        v = self.ln_v(inputs_qkv[..., 2 * s_:])
        qkv = torch.concat((q, k, v), axis=-1)
...
class PerceiverSeq(nn.Module):
    def __init__(self,
                 ...
                 cross_attend_dropout_type="default",
                 autoregression_dropout_keep_prob=1.0,
                 autoregression_dropout_type="default",
                 ...
                 internal_offset_random_seed=0):
        super(PerceiverSeq, self).__init__()
        ...
        self._cross_attend_dropout_type = cross_attend_dropout_type
        self._cross_attend_dropout_keep_prob = cross_attend_dropout_keep_prob

        self._autoregression_dropout_type = autoregression_dropout_type
        ...
        self._internal_offset_random_seed = internal_offset_random_seed

        self._dropout_random_state = np.random.RandomState(4321 + internal_offset_random_seed)
        self._init_random_state = np.random.RandomState(1234 + internal_offset_random_seed)
        self._init_generator_seed = self._init_random_state.randint(100000)

        self._cross_attn_heads = cross_attn_heads
        self._self_attn_heads = self_attn_heads
        ...
        self.input_embed = EmbeddingDropout(self._n_classes, self._input_embed_dim,
                                     dropout_keep_prob=self._input_dropout_keep_prob,
                                     random_state=self._init_random_state,
                                     device=self.device,
                                     dtype=self.dtype)

        self._configuration_embed_dim = self._input_embed_dim
        self.input_query_embed = EmbeddingDropout(self._n_classes, self._configuration_embed_dim,
                                     dropout_keep_prob=self._input_dropout_keep_prob,
                                     random_state=self._init_random_state,
                                     device=self.device,
                                     dtype=self.dtype)

        if self._learnable_position_embeddings:
            self.input_query_pos_embed = Embedding(self._z_index_dim, self._input_embed_dim,
                                                   random_state=self._init_random_state,
                                                   device=self.device,
                                                   dtype=self.dtype)

        if self._autoregressive_mode == False:
            self.mask_merge_linear = nn.Linear(2 * self._input_embed_dim,
        ...
        self.prefinal_layer = nn.Linear(self._num_z_channels,
                                        self._num_z_channels,
                                        bias=True,
                                        device=self.device,
                                        dtype=self.dtype)
        ...
        if self._tied_embeddings:
            assert self._num_z_channels == self._input_embed_dim
            assert self._configuration_embed_dim == self._num_z_channels
            self.final_layer.weight = self.input_query_embed.th_embed.weight
            self.input_embed.th_embed.weight = self.input_query_embed.th_embed.weight
        ...
        self.initial_cross_attn = CrossAttention(self._num_z_channels,
                                                 ...
                                                 fraction_heads_to_rotate=1.0,
                                                 n_heads=self._cross_attn_heads,
    ...
    def _build_network_inputs(self, inputs, input_idxs):
        ...
        embedded_inputs_for_input_out = embedded_inputs_for_input[0]
        embedded_inputs_for_query_out = embedded_inputs_for_query[0]
        # mask embedding directly? or no
        return embedded_inputs_for_input_out, embedded_inputs_for_query_out, positions

    def forward(self, inputs, input_idxs, input_mask=None):
        if self._autoregressive_mode is True:
            if input_mask is not None:
                raise ValueError("input_mask must be None for autoregressive_mask=True")
        else:
            if input_mask is None:
                raise ValueError("Must provide input_mask for autoregressive_mask=False")
        ...
        last_step_embeddings = torch.gather(embedded_inputs_query, 0, latent_last_steps.T[..., None].repeat(1, 1, self._configuration_embed_dim).long())
        ...
        if input_mask is not None:
            ...
            combined_embeddings = torch.concat([last_step_embeddings, input_mask_embed], axis=-1)
            last_step_embeddings = self.mask_merge_linear(combined_embeddings)

        if not self._autoregressive_mode:
            # now that we've concatenated, set the masks to all allow
            # masks[0] is the combined cross attn mask 
            # masks[0][:, 0, 0], [:, 0, 1], etc show this
            # masks[1] is the query mask
            # masks[1][:, 0, 0] shows this
            # set encoder masks to allow all here?
            ...
            # make processor mask allow all - since SUNMASK outer wrapper
        ...
        # generate autoregression mask first, so that cross-attend does not "see" droppped out AR elements
        # latent, batch, hid
        # do AR drop locked dropout style
        # same mask all layers
        # drop elements out of encoder cross attend mask
        if self.training and (self._autoregression_dropout_type != "default" or self._autoregression_dropout_keep_prob != 1.0):
            pm = masks[1].data.new(*masks[1][:, :, 0].size()).zero_()
            if self._autoregression_dropout_type == "default":
                pm = 0. * pm + self._autoregression_dropout_keep_prob
            elif self._autoregression_dropout_type == "uniform":
                keep_p = self._dropout_random_state.rand(int(pm.shape[1]))
                pm = 0. * pm + torch.Tensor(keep_p[None, :]).to(pm.device)
            else:
                raise AttributeError("Unknown dropout type {} specified for self.autoregression_dropout_type".format(self._cross_attend_dropout_type))
            m = torch.bernoulli(pm, generator=self.g)
            if self._autoregression_dropout_type == "uniform":
                apply_rate = self._dropout_random_state.rand(int(pm.shape[1]))
                apply_rate_mask = 1.0 * (apply_rate > self._cross_attend_dropout_keep_prob)
                new_m = ((m + torch.Tensor(apply_rate_mask[None]).to(pm.device))).type(pm.dtype).clip(0, 1)
                m = new_m
            base_drop_decoder_mask = m
            drop_decoder_mask = (m[..., None] * masks[1]).type(masks[1].dtype)
        else:
            base_drop_decoder_mask = masks[1][:, :, 0]
            drop_decoder_mask = masks[1]

        # do cross attend dropout here, accounting for already dropped AR mask
        # drop elements out of encoder cross attend mask
        if self.training and (self._cross_attend_dropout_type != "default" or self._cross_attend_dropout_keep_prob != 1.0):
            pm = masks[0].data.new(*masks[0][:, :, 0].size()).zero_()
            # need to alter the size of this then broadcast...
            # so we have the same dropout over all steps?
            # masks[0] is total size x batch x latent size
            # this means [:, 0, 0] is all elements the 0th step of the 0th batch el can see
            # 1 is allow, 0 block
            # masks[1] is formatted similarly
            if self._cross_attend_dropout_type == "default":
                pm = 0. * pm + self._cross_attend_dropout_keep_prob
            elif self._cross_attend_dropout_type == "uniform":
                keep_p = self._dropout_random_state.rand(int(pm.shape[1]))
                pm = 0. * pm + torch.Tensor(keep_p[None, :]).to(pm.device)
            else:
                raise AttributeError("Unknown dropout type {} specified for self.cross_attend_dropout_type".format(self._cross_attend_dropout_type))
            m = torch.bernoulli(pm, generator=self.g)
            if self._cross_attend_dropout_type == "uniform":
                apply_rate = self._dropout_random_state.rand(int(pm.shape[1]))
                apply_rate_mask = 1.0 * (apply_rate > self._cross_attend_dropout_keep_prob)
                new_m = ((m + torch.Tensor(apply_rate_mask[None]).to(pm.device))).type(pm.dtype).clip(0, 1)
                m = new_m

            # backfill using the AR mask - we do not want to double drop AR part of cross-attend
            base_drop_encoder_mask = m
            dec_mask_len = base_drop_decoder_mask.shape[0]
            # set the back half of this mask to use AR mask already generated
            base_drop_encoder_mask[-dec_mask_len:] = base_drop_decoder_mask
            drop_encoder_mask = (base_drop_encoder_mask[..., None] * masks[0]).type(masks[0].dtype)
            # set the part that goes over latent to be the same as base mask...
        else:
            base_drop_encoder_mask = masks[0][:, :, 0]
            drop_encoder_mask = masks[0]

        if self._learnable_position_embeddings == True:
            rr = torch.arange(0, self._z_index_dim,
                              device=initial_q_input.device,
                              dtype=initial_q_input.dtype)
            embedded_pos_for_query = self.input_query_pos_embed(rr)
            initial_q_input = embedded_pos_for_query[0][:, None] + initial_q_input

        # use the masks in training
        ...
        """
        # append the cross-attend inputs, doubling the length of the sequence...
        # we do an all-allow prefixlm type thing here, could consider instead a decoder only style
        z = torch.concat((embedded_inputs_inputs[:-initial_q_input.shape[0]], z), axis=0)
        drop_decoder_mask_prop = torch.concat([torch.triu(torch.ones(200, 200))[:, None] for _ in range(drop_decoder_mask.shape[1])], axis=1).to(drop_decoder_mask.device)
        drop_decoder_mask_prop[:, :, :drop_encoder_mask.shape[-1]] = drop_decoder_mask_prop[:, :, :drop_encoder_mask.shape[-1]] * 0 + 1
        drop_decoder_mask_prop[drop_encoder_mask.shape[-1]:, :, :drop_encoder_mask.shape[-1]] = drop_decoder_mask_prop[drop_encoder_mask.shape[-1]:, :, :drop_encoder_mask.shape[-1]] * 0
        drop_decoder_mask = drop_decoder_mask_prop
        # this does't actually have dropout yet........
        """
        ...
        if self._learnable_position_embeddings == True:
            rr = torch.arange(0, self._z_index_dim,
                              ...
                              dtype=z.dtype)
            embedded_pos_for_query = self.input_query_pos_embed(rr)
            z = embedded_pos_for_query[0][:, None] + z
        ...
        for processor_l in self.processor_layers:
            z, initial_cross_attend_state = processor_l(
                ...
                #kv_positions=positions)
        # take the slices related to the query
        z = z[-initial_q_input.shape[0]:]
        z = self.ln_o(z)
        # drop before prefinal. other models the drop is before the output layer but here we have param sharing with input emb
        z = self.drop(z)
        pre_o = self.prefinal_layer(z)
        out = self.final_layer(pre_o)
        return out, masks, (drop_encoder_mask, drop_decoder_mask), (base_drop_encoder_mask, base_drop_decoder_mask)
